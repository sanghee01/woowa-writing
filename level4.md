...
Sentry Issues 페이지는 에러 통계, 브라우저/OS 정보, Stack Trace 등 기본 정보를 제공합니다. 하지만 실제 사용하면서 두 가지 치명적인 문제를 발견했습니다.
...
URL이 `/posts`라고 해도 게시글 목록을 불러오다 발생한 건지, 작성 중 발생한 건지, 삭제 중 발생한 건지 알 수 없었습니다. 사용자 행동, 처리 중이던 데이터 같은 컨텍스트 정보가 없으니 원인 파악이 정말 어려웠습니다.

### 1.3 미니 프로젝트로 실험하기

팀 프로젝트에 바로 적용할 수도 있었지만, PR과 코드 리뷰 과정을 거쳐야 하고 실험적인 시도를 하기에는 부담스러웠습니다. 그래서 프로젝트 환경과 최대한 비슷한 미니 프로젝트를 만들어 하나씩 실험하며 적용했습니다.

이렇게 하면 다음과 같은 장점이 있습니다.

- 빠르게 시도하고 결과를 확인할 수 있습니다.
- 실패해도 팀에 영향을 주지 않습니다.
- 검증된 방법만 팀 프로젝트에 적용할 수 있습니다.
